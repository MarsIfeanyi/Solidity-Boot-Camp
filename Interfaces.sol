...
INTERFACES 
helps contract to interact with another contract without copy and pasting the codes.
Interfaces are similar to abstract contracts and are created using interface keyword. 
The following are the key characteristics of an interface.
...
//with INTERFACES, we can we write a contract that interacts with this contract (Counter contract) without copy pasting the source code.
...
/**Hint: A liquidity pool is a collection of funds locked in a smart contract. Liquidity pools are used to facilitate decentralized trading, lending, and many more functions.
Liquidity pools are the backbone of many decentralized exchanges (DEX), such as Uniswap. Users called liquidity providers (LP) add an equal value of two tokens in a pool to create a market */

interface UniswapV2Factory {
    // Function signature to get pairs (gotten from the uniswap doc v2)
...
interface UniswapV2Pair {
    //function signature to get reserve values
...
contract UniswapExercise {
    ...
    //function to get the pair of tokens and set it to a single address
    function getTokenReserves() external view returns (uint256, uint256) {
        address pair = UniswapV2Factory(factory).getPair(DAI, WETH9);
        //Using the address to get the reserve values and finally returning the reserve values
        (uint256 reserveA, uint256 reserveB) = UniswapV2Pair(pair)
            .getReserves();
